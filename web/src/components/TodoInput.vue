                    ...
                    <form @submit.prevent="submit">
                        <div class="row">
                            <div class="input-field col s7">
                                <input id="body-input" type="text" :class="{invalid: !isTodoInputValid}" placeholder="buy milk" v-model="todoInput">
                                <label data-error="required" class="active" for="body-input">New Todo</label>
                            </div>
                            <div class="input-field col s3">
                                <input id="due-date-input" type="text" :class="{invalid: !isDueDateInputValid}" placeholder="YYYY-MM-DD" v-model="dueDateInput">
                                <label data-error="YYYY-MM-DD" class="active" for="due-date-input">Due Date</label>
                            </div>

                            <div class="col s2">
                                <a class="btn-floating btn-large waves-effect waves-light brown dark" @click="submit"><i class="material-icons">add</i></a>
                            </div>
                        </div>
                    </form>
                </div>
